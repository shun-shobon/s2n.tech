...
.repeat-examples {
  ...
  /* 自動繰り返し */
  grid-template-columns: repeat(auto-fill, 200px); /* 200pxの列を可能な限り配置 */
...
.grid-debug {
  ...
  /* グリッドラインを表示（デバッグ用） */
  background-image: 
    linear-gradient(to right, rgba(255, 0, 0, 0.1) 0%, rgba(255, 0, 0, 0.1) 100%),
    linear-gradient(to bottom, rgba(0, 0, 255, 0.1) 0%, rgba(0, 0, 255, 0.1) 100%);
  background-size: calc(100% / 12) 100%, 100% 50px;
}
```
